...
@section('buttons')
  ...
  @endphp
  @if ($new != 1)
    <a href='{{ url('webservices/extension/new') }}' class="btn btn-app bg-blue"><li class="fa fa-phone"></li> Nueva Extensión</a>
    ...
    <form id="allow" action="{{ action('\App\Modules\WebServices\Controllers\ExtensionController@allowExtension', array('id' => $entity->id, 'active' => !$entity->active)) }}" method="POST" style="display: none;">
      {{ csrf_field() }}
    </form>
    <a href='#' class="btn btn-app btn-password bg-blue"><i class="fa fa-asterisk"></i>Establecer Contraseña</a>
    <form id="password" action="{{ action('\App\Modules\WebServices\Controllers\ExtensionController@changePassword', array('id' => $entity->id)) }}" method="POST" style="display: none;">
      {{ csrf_field() }}
  ...
  <div class="pull-right">
    @if (Auth::check())
      ...
      <a class="btn btn-app" href='{{ url('/webservices/extensiones') }}'><span class="badge bg-purple">{{ $entity::all()->count() }}</span><i class="fa fa-list"></i> Volver</a>
    @endif
  </div>
...
@section('breadcrumb')
  ...
  <li><a href="{{ url('/webservices') }}"><i class="fa fa-money"></i> WebServices</a></li>
  <li><a href="{{ url('/webservices/extensiones') }}"><i class="fa fa-phone"></i> Extensiones</a></li>
...
<section class="content">
  <div class="row">
    <div class="col-xs-12">
      <form id="extension" action="{{ $new == 1 ? action('\App\Modules\WebServices\Controllers\ExtensionController@postEntity') : action('\App\Modules\WebServices\Controllers\ExtensionController@putEntity', $entity->id) }}" method="post" class="form-horizontal" enctype="multipart/form-data"  onkeypress="return checkSubmit(event, this)">
        {{ method_field('PUT') }}
        {{ csrf_field() }}
        ...
        <div class="nav-tabs-custom">
          <ul class="nav nav-tabs">
            @php
              ...
              if ($errors_found)
                $group = App\Modules\WebServices\Controllers\ExtensionController::groupHasErrors($inputs, $errors);
            @endphp
...
@section('jquery_onload')
  ...
  {{ AdminLTE::Menu_Active("webservices_extensiones") }}
  @if ($new)
